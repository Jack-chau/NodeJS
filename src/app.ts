...
import { Customer } from './models/customers' ; // Typescript
import Supplier from './models/suppliers' ; // same as Customer, but you can change name here

import { Request, Response } from 'express' ; // import datatype while using TypeScript
...
app.get( '/api/customers', async( request : Request, response : Response ) => {
...
// For supplier
app.get( '/api/suppliers', async( request : Request, response : Response ) => {
    try {
...
app.post( '/api/suppliers', async( request : Request, response : Response ) => {
    console.log( request.body ) ;
...
app.get( '/api/suppliers/:id', async( request : Request, response : Response ) => {
    try{ 
...
app.put( '/api/customers/:id', async( request : Request, response : Response ) => {
    try {
...
app.put( '/api/suppliers/:id', async( request : Request, response : Response ) => {
    try{ 
        const { id : supplierId } = request.params ;
...
// Implement Patch
app.patch( '/api/customers/:id', async( request : Request, response : Response ) => {
    try{
        const { id : customerId } = request.params ;
        const customer = await Customer.findOneAndUpdate( { _id : customerId }, request.body, { new : true } ) ;
...
app.patch( '/api/order/:id', async( request : Request, response : Response ) => {
...
app.patch( '/api/suppliers/:id', async( request : Request, response : Response ) => {
    try{
        const { id : supplierId } = request.params ;
        const supplier = await Supplier.findOneAndReplace( { _id : supplierId }, request.body, { new : true } ) ;
...
// Delete items
app.delete( 'api/customers/:id', async( request : Request, response : Response ) => {
    try {
        const customerId = request.params.id ;
...
app.delete( '/api/suppliers/:id', async( request : Request, response : Response ) => {
    try{
        const { id : supplierId } = request.params ;
